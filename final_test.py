...
# Main function to simulate multiple WebSocket connections
async def main(num_users=1000):
    """Simulate multiple WebSocket connections."""
    
    tasks = []
    
    # Loop through and create multiple users
    for _ in range(num_users):
        ...
        # Simulate login and get token and user_id
        token, user_id = login(email, password)
        
        if token and user_id:
            # Add the WebSocket chat task to the task list
            tasks.append(websocket_chat(token, user_id))
    
    # Run all WebSocket chat tasks asynchronously
    await asyncio.gather(*tasks)

if __name__ == "__main__":
    start_time = time.time()
    asyncio.run(main(num_users=1000))  # Set the number of WebSocket connections here (e.g., 1000)
